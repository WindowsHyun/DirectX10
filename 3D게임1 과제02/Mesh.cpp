...
CBigCubeMesh::CBigCubeMesh( ID3D11Device *pd3dDevice, float fWidth, float fHeight, float fDepth ) : CMesh( pd3dDevice )
{
	m_nVertices = 8;
	m_nStride = sizeof( CDiffusedVertex );
	m_nOffset = 0;
	...
	float fx = fWidth*1.3f, fy = fHeight*1.3f, fz = fDepth*1.3f;
	...
	// 따라하기(11)로 진행함.
	UINT pIndices[20];
	pIndices[0] = 3; pIndices[1] = 2; pIndices[2] = 1; pIndices[3] = 0;

	pIndices[4] = 4; pIndices[5] = 5; pIndices[6] = 1; pIndices[7] = 0;

	pIndices[8] = 4; pIndices[9] = 7; pIndices[10] = 3; pIndices[11] = 0;

	pIndices[12] = 3; pIndices[13] = 2; pIndices[14] = 6; pIndices[15] = 7;

	pIndices[16] = 6; pIndices[17] = 5; pIndices[18] = 1; pIndices[19] = 2;

	/*pIndices[20] = 4; pIndices[21] = 7; pIndices[22] = 6; pIndices[23] = 0;*/
	ZeroMemory( &d3dBufferDesc, sizeof( D3D11_BUFFER_DESC ) );
	d3dBufferDesc.Usage = D3D11_USAGE_DEFAULT;
	d3dBufferDesc.ByteWidth = sizeof( UINT ) * m_nIndices;
	d3dBufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
...
CBigCubeMesh::~CBigCubeMesh()
{
}

void CBigCubeMesh::CreateRasterizerState( ID3D11Device *pd3dDevice )
{
	D3D11_RASTERIZER_DESC d3dRasterizerDesc;
	ZeroMemory( &d3dRasterizerDesc, sizeof( D3D11_RASTERIZER_DESC ) );
	d3dRasterizerDesc.CullMode = D3D11_CULL_NONE;
...
void CBigCubeMesh::Render( ID3D11DeviceContext *pd3dDeviceContext )
{
	CMesh::Render( pd3dDeviceContext );
}