...
void CGameFramework::OnProcessingKeyboardMessage( HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam ) {
	...
	case WM_KEYDOWN:
		...
		case VK_NUMPAD0:
			...
			for ( int i = 0; i < c_Value; ++i ) {
				float diffX = c_X[i] - p_X;
				float diffZ = c_Z[i] - p_Z;
				float result = sqrt( diffX * diffX + diffZ * diffZ );
				if ( result <= 100 ) {
					c_View[i] = false;
				}
			}
	...
	case WM_KEYUP:
		...
		case VK_F3:
			//if ( m_pPlayer )
			//{
			//	m_pPlayer->ChangeCamera( m_pd3dDevice, (0x72 - 0x70 + 1), m_GameTimer.GetTimeElapsed() );
